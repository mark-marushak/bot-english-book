...
func (s StudyStart) Keyboard(i ...interface{}) interface{} {
	return tgbotapi.NewReplyKeyboard(tgbotapi.NewKeyboardButtonRow(tgbotapi.NewKeyboardButtonContact("Next Lesson")))
}
...
func (s StudyStart) Output(i ...interface{}) (string, error) {
	...
	poll := tgbotapi.NewPoll(s.GetUpdate().FromChat().ID, fmt.Sprintf("Translate this %s", options[0]), options...)
	poll.CorrectOptionID = 0
	poll.Type = "quiz"
	poll.AllowsMultipleAnswers = true

	s.GetBotAPI().Send(poll)
